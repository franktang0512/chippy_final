...
async function init_canvas()
{
	...
	curX = 0, curY = 2;			// (curX, curY):青蛙位置, jump:呈現跳躍姿態, drop:是否掉進水裡
	index = 1, dir = [[-1,0],[0,-1],[1,0],[0,1]];				// index:方向, dir:順時針方向變化[左、上、右、下]
	// imgName[是否為移除陷阱]
	imgName = []
	// 順時針圖片變化[左、上、右、下]
	imgName[true] = ["libs/Advanture/chippy_trap-left.svg","libs/Advanture/chippy_trap-up.svg","libs/Advanture/chippy_trap-right.svg","libs/Advanture/chippy_trap-down.svg"];
	imgName[false] = ["libs/Advanture/chippy-left.svg","libs/Advanture/chippy-up.svg","libs/Advanture/chippy-right.svg","libs/Advanture/chippy-down.svg"];
	clear = []
...
function draw(onsize = false) 
{
	...
	for(var x = 0; x < matrix[0].length; ++x){
		for(var y = 0; y < matrix.length; ++y){
			...
			{
				img.src = "libs/Advanture/trap.svg";
				img.data['trap']=true;
			...
			{
				img.src = "libs/Advanture/tree.svg";
			}
			else if(matrix[y][x] == 'g')
			{
				img.src = "libs/Advanture/goal.svg";
			}
		}
	}
	// 放置Chippy
	...
	{
		img.data.fail=true;
		img.src = "libs/Advanture/chippy_fail.svg";
	}
