...
<body>
		...
		<div class='container'>
			...
			<button id='file_id_submit' class='submit' onClick='dbdisplay_en()'>讀資料庫(encode)</button>
			
			<input id='file_id' type='text' placeholder='filename'>
			<button id='file_id_submit' class='submit' onClick='fidisplay()'>讀檔</button>
			<button id='file_id_submit' class='submit' onClick='fidisplay_en()'>讀檔(encode)</button>
			<button id='file_id_submit' class='submit' onClick='debug_en()'>除錯(encode)</button>
	...
	<div class="workspace" id="blocklyDiv_en"></div>
    <div class="workspace" id="scratchDiv"></div>	
    <div class="workspace" id="scratchDiv_en"></div>
    ...
    <script>
		...
		function init() {
			...
			workspace_blockly_en = Blockly.inject('blocklyDiv_en',
            {
                toolbox: toolbox_scratch,
                zoom: 
                {
            ...
            workspace_scratch_en = Blockly.inject('scratchDiv_en',
            {
                toolbox: toolbox_scratch,
                zoom: 
                {
	...
	function dbdisplay(){
		...
		xmlhttp.open("POST", "get_student_result_by_sa_id_.php", true);
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send("func=d&sa="+document.getElementById("db_id").value);
	}
	
	
	function dbdisplay_en(){
		
		workspace_blockly_en.clear();
		//alert(document.getElementById("file_id").value);
		const xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() {
			if (this.readyState === 4 && this.status === 200) {
				...
				if (this.responseText.trim() != "") {
					...
					// //xml字串轉blockly xml物件
					var xml_object = Blockly.Xml.textToDom(''+response);
					Blockly.Xml.domToWorkspace(xml_object, workspace_blockly_en);
		...
		xmlhttp.open("POST", "get_student_result_by_sa_id_.php", true);
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send("func=de&sa="+document.getElementById("db_id").value);
	}
	...
	function fidisplay_en(){
		workspace_scratch_en.clear();
		//alert(document.getElementById("file_id").value);
		const xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() {
			if (this.readyState === 4 && this.status === 200) {
				...
				if (this.responseText.trim() != "") {
					...
					// //xml字串轉blockly xml物件
					var xml_object = Blockly.Xml.textToDom(""+response);
					Blockly.Xml.domToWorkspace(xml_object, workspace_scratch_en);
		...
		xmlhttp.open("POST", "get_student_result_by_sa_id_.php", true);
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send("func=fe&sa="+document.getElementById("file_id").value);
	}
	...
	function debug_en(){
		...
		xmlhttp.open("POST", "get_student_result_by_sa_id_.php", true);
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send("func=dfe&sa="+document.getElementById("file_id").value);
	}
