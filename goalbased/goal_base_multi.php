...
<body>
        ...
        <div id="question_div" style="display: block; height: 92%; width: 100%;">
            <div style="background-image: url('./libs/blackboard_back.png'); display: block; padding: 5%; height: 90%;">
                <div id="board_div" style="background-color: rgb(92, 131, 84); padding: 2%; display: block;height: 100%;">
                    <textarea id="question" class="text" readonly style="color: white; height: 100%;"><?php echo $e_description?>
                    </textarea>
                </div>   
            </div>
    ...
    <script>
        ...
        // 初始化畫布
        var finish = [];			//設置以區隔呼叫detectResult或detectResultFinish
        ...
        var runner = [];
        var stoID = [];
        var conguration = false;

        // 停止程式碼
        function runner_func(i) {
            if(myInterpreter[i]) 
            {
                var hasMore = myInterpreter[i].run();
                if(hasMore)stoID[i]=setTimeout(runner[i], 10);
                else
                {
                    resetInterpreter(i);
                    finish[i] = true;
                    draw();
                    if(!finish.includes(false) && correct && !conguration)
                    {
                        conguration=true;
                        changeBtn();
                        firework();
        ...
        // 停止程式碼
        function resetInterpreter(n) 
        {
            myInterpreter[n] = null;
            if(runner[n]) 
            {
                clearTimeout(stoID[n]);
                runner[n] = null;
            }
        }
        function resetAllInterpreter() 
        {
            for(i in myInterpreter){
                resetInterpreter(i);
                finish[i]=false;
            }
        }
            ...
            try 
            {
                setTimeout(function() 
                {
                    for(var i=0;i<cartNum;i++) {
                        runner[i]= runner_func.bind(null,i);
                    }
                    for(var i=0;i<cartNum;i++){
                        myInterpreter[i] = new Interpreter("var cart="+i+";"+code, initApi);
                        runner[i]();
                    }
                }, 1);
            } 
