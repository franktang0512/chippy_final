...
<head>
    ...
    <script src="../tasks_statement_file/<?php echo $e_id;?>/question.js"></script>
	<script>
		var task_info;
	
		getquestioninfo();
		var map_id =<?php echo $mapversion;?>;
		...
		if($_SESSION["u_level"]=="0"){
			$u_name =$_SESSION["stu_id"];
                 ...
                 <div class='d-flex flex-wrap justify-content-right'>
					...
					".
						($_SESSION["u_level"]=='1'||$_SESSION["u_level"]=='2'?"<ul><button id='stulist' onclick='openstudentlist();'class=''>學生名單</button>
...
<body style="position: relative;">
    ...
    <div id="view" class="column canvas-bg" style="position: relative;">
        ...
        <div id="canvas_div" style="display: none;">
            <img id="info_img" width="100%" src="libs/ship/info.png" style="aspect-ratio: 0.9;">
            <img id="chal_img" width="100%" src="libs/ship/chal.png" style="aspect-ratio: 1.2; display: none">
            ...
            <div style="text-align:center;">
                ...
                <button id="sent" class="btn btn-lg" style="background-color: blue; color: white;" onclick="submit_answer();">送出成果</button>
            </div>
            <div id="how_many" style="text-align:center;">
                <p><b>已使用 <span id="num"></span> 塊積木，剩餘 <span id="capacity"></span> 塊積木</b></p>
            </div>
            
        </div>
    </div>
    <div id="long_view">
        <div id="long_canvas_div" style="height:100%; width:100%;">
            <canvas id="long_draw" height="1024" width="1024"></canvas>
        </div>
    ...
    <script>
		...
		var img_src = "../tasks_statement_file/<?php echo $e_id;?>";
        init()
        function init() {
		...
		function max_number(event) {
            document.getElementById("capacity").textContent =
            current_workspace.remainingCapacity();
            document.getElementById("num").textContent =
            current_workspace.getAllBlocks().length;
        }
        workspace_blockly.addChangeListener(max_number)
        workspace_scratch.addChangeListener(max_number)
        ...
        var canvas = document.getElementById("long_draw");
        ...
        function show_question() {
            ...
            document.getElementById('long_view').style.display='none';
			const xmlhttp = new XMLHttpRequest();
			xmlhttp.onreadystatechange = function() {
				if (this.readyState === 4 && this.status === 200) {
        ...
        function show_canvas() {
			...
			document.getElementById('how_many').style.display='inline';
            document.getElementById('long_view').style.display='block';
            document.getElementById('info_img').style.display='none';
            document.getElementById('chal_img').style.display='block';
			const xmlhttp = new XMLHttpRequest();
			xmlhttp.onreadystatechange = function() {
				if (this.readyState === 4 && this.status === 200) {
        ...
        function show_example() 
        {
            ...
            document.getElementById('info_img').style.display='block';
            document.getElementById('chal_img').style.display='none';
			const xmlhttp = new XMLHttpRequest();
			xmlhttp.onreadystatechange = function() {
				if (this.readyState === 4 && this.status === 200) {
        ...
        function changeBtn() 
        {
            ...
            else
            {
                clearAll();
                init_canvas();
                document.getElementById("view").style.cssText = "background-color: transparent"
            }
			const xmlhttp = new XMLHttpRequest();
			xmlhttp.onreadystatechange = function() {
				if (this.readyState === 4 && this.status === 200) {
...
<?php write_chippy_log($conn,"目標導向題目頁面");?>